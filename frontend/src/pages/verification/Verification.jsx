...
export default function Verification() {
  ...
  const fetchApproved = async () => {
    ...
    try {
      ...
      if (data.success) {
        // Filter records that have been approved (have overallStatus === 'approved' or have document_id)
  ...
  const handleRecordSelect = (record) => {
    ...
    if (record.document_id) {
      ...
      setVerificationResult(null);
      setVerificationError("No cryptographically signed Document ID code generated for this record.");
    }
  };
  ...
  // Helper to format field values safely
  const formatFieldVal = (field) => {
    const val = typeof field === 'object' && field !== null && 'value' in field ? field.value : field;
  ...
  if (dbError) {
    return (
      <div className="flex-1 p-6 md:p-12 flex flex-col items-center justify-center bg-slate-900 text-slate-100">
        <div className="max-w-md w-full bg-slate-950 border border-rose-500/30 rounded-2xl p-8 text-center space-y-5 shadow-2xl">
          ...
          <p className="text-sm text-slate-400 leading-relaxed">
            The PostgreSQL database is currently offline. Active registry records cannot be fetched or verified.
          </p>
          <button
    ...
    <div className="flex-1 p-4 md:p-8 overflow-y-auto bg-slate-900 text-slate-100 min-h-0">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="border-b border-slate-800 pb-6">
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-white">Registry Verification Center</h2>
          <p className="text-sm text-slate-400 mt-2">
            Stage 5: Cryptographically verify signed land documents and public registry records against the tamper-proof ledger.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          
          {/* Left Column: Approved Documents Browser & Certificate Uploader */}
          <div className="lg:col-span-1 space-y-6">
            <div className="space-y-4">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Approved Registries</h3>
              ...
              <form onSubmit={handleSearchSubmit} className="flex space-x-2">
                <input
                  ...
                  className="flex-1 bg-slate-950 border border-slate-800 text-xs rounded-lg px-3 py-2 text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
                <button
                  type="submit"
              ...
              <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
                {loading ? (
                  <div className="flex flex-col items-center justify-center py-8 space-y-2">
                    ...
                    <span className="text-xs text-slate-500">Loading approved records...</span>
                  </div>
                  ...
                  approvedRecords.map((rec) => (
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                        <span className="font-mono font-semibold text-slate-200 truncate max-w-[150px]">
                          {rec.document_id || 'Approved Record'}
                        </span>
                      ...
                      <div className="flex justify-between w-full text-[10px] text-slate-500">
                        ...
                        <span>{rec.fields?.village?.value || rec.district || '—'}</span>
                      </div>
                    </button>
                  ))
                ) : (
                  <div className="p-8 text-center text-slate-500 text-xs border border-slate-850 rounded-xl border-dashed">
            ...
            <div className="border border-slate-800/60 bg-slate-950/40 rounded-2xl p-5 space-y-4 shadow-md">
              ...
              <div className="flex flex-col space-y-3">
                ...
                {uploadedCert && (
                  <div className="p-3 bg-slate-900/60 border border-slate-800 rounded-lg space-y-3">
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-[11px] font-semibold text-white rounded-md transition-colors"
                    >
                ...
                {certVerifyError && (
                  <div className="p-3 bg-rose-950/20 border border-rose-900/40 rounded-lg text-[10px] text-rose-400 font-mono">
                    Failed: {certVerifyError}
                  </div>
                )}

                {certVerifyResult && (
                  <div className={`p-3 border rounded-lg text-center font-bold text-[11px] shadow-sm ${
          ...
          {/* Right Column: Cryptographic Verification Display panel */}
          <div className="lg:col-span-2 space-y-6">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">Cryptographic Details</h3>
            
            {verificationLoading ? (
              <div className="p-16 text-center text-slate-500 text-xs border border-slate-800 rounded-2xl bg-slate-950/20 flex flex-col items-center justify-center space-y-3">
                <div className="w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
                <span>Contacting Verification-Mark microservice...</span>
              </div>
            ) : verificationResult ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                ...
                <div className="md:col-span-1 space-y-4">
                  ...
                  {selectedRecord?.qr_code && (
                    <div className="border border-slate-800 bg-slate-950 rounded-2xl p-4 flex flex-col items-center justify-center text-center space-y-3.5 shadow-md">
                      ...
                      {selectedRecord.document_id && (
                        <a
                          href={`?view=public-verify&id=${selectedRecord.document_id}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-[11px] text-blue-400 hover:text-blue-300 font-semibold underline flex items-center justify-center space-x-1"
                        >
                ...
                <div className="md:col-span-2 space-y-6">
                  ...
                  <div className="bg-slate-950/40 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-md">
                    <h4 className="font-semibold text-slate-200 text-xs uppercase tracking-wider border-b border-slate-800/80 pb-2">
                      Signed Data Fields
                    </h4>
                      ...
                      {Object.keys(verificationResult.record?.fields || selectedRecord?.fields || {}).map((key) => {
                        ...
                        return (
                          <div key={key} className="space-y-1">
                            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold block">{key.replace(/_/g, ' ')}</span>
                            <div className="font-medium text-slate-300 truncate">{formatFieldVal(field)}</div>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="bg-slate-950/20 border border-slate-800 rounded-2xl p-5 space-y-4 text-[10px] font-mono text-slate-400">
                    <h4 className="font-semibold text-slate-200 text-xs uppercase tracking-wider font-sans border-b border-slate-850 pb-2">
                      Signature Blocks
                    </h4>
                    <div className="space-y-3">
                      <div>
                        <span className="text-slate-500 font-bold block mb-1">DIGITAL SIGNATURE BLOCK (HEX)</span>
                        <div className="bg-slate-950 border border-slate-850 p-2.5 rounded-lg max-h-16 overflow-y-auto break-all font-mono text-[10px] text-slate-300">
              ...
              <div className="p-16 text-center text-slate-500 text-xs border border-slate-800 border-dashed rounded-2xl bg-slate-950/10">
                Select an approved record from the left browser or search for a Document ID to check signature verification.
              </div>
            )}
            
            {verificationError && (
              <div className="p-4 bg-rose-950/20 border border-rose-900/40 rounded-xl text-xs text-rose-400 font-mono">
