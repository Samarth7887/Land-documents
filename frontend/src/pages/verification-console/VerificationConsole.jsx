...
export default function VerificationConsole() {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      console.warn("Could not fetch history from backend, falling back to empty list.", err);
    }
  };
  ...
  const getConfidenceBand = (score) => {
    if (score >= 0.9) return { label: "Auto Approved", color: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20", band: "auto_approved" };
    if (score >= 0.6) return { label: "Needs Review", color: "bg-amber-500/10 text-amber-400 border-amber-500/20", band: "needs_review" };
    return { label: "Needs Correction", color: "bg-rose-500/10 text-rose-400 border-rose-500/20", band: "needs_correction" };
  };
  ...
  const handleFieldChange = (key, value) => {
    if (!activeRecord?.fields?.[key]) return;
  ...
  const handleSaveCorrections = async () => {
    ...
    Object.keys(formFields).forEach((key) => {
      ...
      if (original && String(current.value) !== String(original.original_value)) {
        logs.push({
          field: key,
  ...
  if (dbError) {
    return (
      <div className="flex-1 p-6 md:p-12 flex flex-col items-center justify-center bg-slate-900 text-slate-100 min-h-screen">
        <div className="max-w-md w-full bg-slate-950 border border-rose-500/30 rounded-2xl p-8 text-center space-y-5 shadow-2xl">
          <div className="text-rose-500 text-4xl">⚠️</div>
          <h2 className="text-xl font-bold text-slate-200">Database unavailable</h2>
          <p className="text-sm text-slate-400 leading-relaxed">
            The PostgreSQL database is offline. Real-time land registries and review queues cannot be loaded.
          </p>
          <button
            onClick={fetchRecords}
            className="w-full px-4 py-2.5 bg-slate-800 hover:bg-slate-700 active:bg-slate-700 text-sm rounded-lg text-slate-200 font-semibold transition-colors border border-slate-700"
          >
            Retry Connection
          </button>
        </div>
  ...
  if (isLoading) {
    return (
      <div className="flex-1 p-12 flex flex-col items-center justify-center bg-slate-900 text-slate-100 min-h-screen space-y-3">
        <div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
        <span className="text-sm text-slate-400">Loading records queue...</span>
      </div>
    );
  }

  if (!queue.length || !activeRecord) {
    return (
      <div className="flex-1 p-6 md:p-12 flex flex-col items-center justify-center bg-slate-900 text-slate-100 min-h-screen">
        <div className="max-w-md w-full rounded-2xl border border-slate-800 bg-slate-950/70 p-8 text-center space-y-4 shadow-xl">
          ...
          <button
            onClick={fetchRecords}
            className="w-full px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-sm rounded-lg text-slate-200 font-semibold transition-colors border border-slate-700"
          >
            Refresh Queue
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-68px)] bg-slate-900 text-slate-100 font-sans min-h-0 overflow-hidden">
      
      {/* LEFT COLUMN: Record Queue Sidebar */}
      <aside className="w-full lg:w-80 border-r border-slate-850 bg-slate-950/70 p-4 flex flex-col min-h-0 overflow-hidden">
              ...
              <button
                ...
                className={`w-full text-left p-3.5 rounded-xl border transition-all flex flex-col space-y-2 ${
                  selectedIdx === idx
                    ...
                    : "bg-slate-950/40 border-slate-900 text-slate-400 hover:bg-slate-800/30"
                }`}
              >
                <div className="flex justify-between items-center w-full gap-2">
                  <span className="font-mono font-bold text-sm text-slate-200 truncate">{item.id}</span>
                  <span className={`text-[9px] px-2 py-0.5 rounded-full border ${getStatusClass(item.overallStatus)} font-bold shrink-0`}>
                    {getStatusLabel(item.overallStatus)}
                  </span>
                </div>
                
                <div className="text-xs space-y-0.5">
                  <div className="text-slate-300 font-semibold">{getFieldValue(item, 'village')}</div>
        ...
        <div className="mt-4 border-t border-slate-800 pt-4 flex-1 flex flex-col min-h-[160px] overflow-hidden">
          <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">
            Record History Trail
          </h4>
          
          <div className="flex-1 overflow-y-auto space-y-3.5 pr-1 text-[11px] font-mono scrollbar-none">
            {/* State transitions */}
            {historyTrail.state_transitions && historyTrail.state_transitions.length > 0 && (
              <div className="space-y-1">
                <div className="text-[10px] font-sans font-bold text-slate-500 tracking-wider">STATE TRANSITIONS:</div>
                <div className="space-y-1 bg-slate-950 border border-slate-850 p-2 rounded-lg text-[10px] leading-relaxed">
            ...
            {historyTrail.field_corrections && historyTrail.field_corrections.length > 0 && (
              <div className="space-y-1">
                <div className="text-[10px] font-sans font-bold text-slate-500 tracking-wider">FIELD CORRECTIONS:</div>
                <div className="space-y-2 bg-slate-955 border border-slate-850 p-2.5 rounded-lg text-[10px] bg-slate-950">
                  {historyTrail.field_corrections.map((corr) => (
                    <div key={corr.id} className="text-slate-300 border-b border-slate-900 pb-1.5 last:border-0 last:pb-0">
                      <div className="text-amber-500 font-bold">{corr.field_name}:</div>
                      <div className="pl-1 truncate text-slate-550 line-through">{corr.original_value}</div>
                      <div className="pl-1 truncate text-emerald-400 font-semibold">&rarr; {corr.corrected_value}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {(!historyTrail.state_transitions?.length && !historyTrail.field_corrections?.length) && (
              <div className="text-slate-500 text-center py-6 font-sans text-xs italic">No history trail logged.</div>
            )}
          </div>
        </div>
      </aside>
      ...
      <section className="flex-1 bg-slate-950 relative flex flex-col border-r border-slate-850 min-h-[300px]">
        
        {/* Toolbar */}
        <div className="absolute top-4 left-4 z-10 bg-slate-900/90 border border-slate-800 p-1.5 rounded-xl shadow-lg flex items-center space-x-2 backdrop-blur">
          <button 
            onClick={() => handleZoom(0.2)} 
            className="w-8 h-8 rounded-lg hover:bg-slate-800 flex items-center justify-center text-slate-350 text-base font-bold transition-colors"
            title="Zoom In"
          >
            +
          </button>
          ...
          <button 
            ...
            className="px-2.5 py-1.5 text-xs hover:bg-slate-850 hover:text-slate-200 rounded-lg text-slate-500 transition-all font-semibold"
            title="Reset Zoom"
          >
            Reset
          </button>
          <span className="text-[10px] text-slate-500 px-1.5 font-mono border-l border-slate-800">
            {Math.round(zoom * 100)}%
          </span>
        </div>
        ...
        >
          {activeRecord.imageUrl ? (
            <div 
              ...
              className="relative flex items-center justify-center border border-slate-800 rounded-xl overflow-hidden shadow-2xl bg-slate-900"
            >
              <img
              ...
              {activeRecord.qr_code && (
                <div className="absolute bottom-4 right-4 bg-white/95 border border-slate-300 p-1.5 rounded-lg shadow-lg flex flex-col items-center space-y-1">
                  <img 
      ...
      <section className="w-full lg:w-[500px] bg-slate-950 flex flex-col min-h-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 min-h-0">
          <div className="flex justify-between items-center border-b border-slate-900 pb-3">
            ...
            <button 
              ...
              className="text-xs text-slate-500 hover:text-slate-350 font-semibold underline"
            >
              Reset Form
            </button>
          </div>
              ...
              return (
                <div key={key} className="space-y-2 border-b border-slate-900 pb-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span 
                      className={`text-[9px] font-mono px-2 py-0.5 rounded-full border ${confObj.color} cursor-help transition-colors font-bold uppercase`}
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {confObj.band === "needs_correction" && (
                      <button
                        ...
                        className={`px-3 py-2 text-xs rounded-lg border font-bold transition-colors shrink-0 ${
                          isOverridden
                  ...
                  {field.issue && (
                    <p className="text-[11px] text-rose-450 font-medium pl-1 flex items-center">
                      ⚠️ {field.issue}
                    </p>
                  )}
                </div>
        ...
        <div className="p-6 border-t border-slate-800 bg-slate-950/80 shrink-0 flex justify-end space-x-3 backdrop-blur">
          <button
            onClick={handleSaveCorrections}
            className="px-4 py-2.5 text-xs font-bold rounded-xl border border-slate-800 text-slate-300 hover:bg-slate-900 transition-colors shadow-sm"
          >
            Save Corrections
          </button>
          <button
            ...
            className={`px-5 py-2.5 text-xs rounded-xl font-bold shadow-lg transition-all ${
